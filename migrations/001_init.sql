...
CREATE TABLE IF NOT EXISTS papers (
  ...
  -- embedding + shadow (v1: shadow is logged only, not a gate)
...
CREATE TABLE IF NOT EXISTS arxiv_cursor (
  ...
  last_submission_prefix TEXT, -- e.g. submittedDate lower bound (opaque string we compare)
  last_arxiv_id TEXT
);
...
-- Hand labels log (can have multiple over time; UI uses latest for display)
